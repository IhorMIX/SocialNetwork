...
public class UserPostController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> CreatePost([FromBody] CreateUserPostModel post,
        CancellationToken cancellationToken)
    {
        ...
        var createdPost =
            await _postService.CreateUserPost(userId, _mapper.Map<UserPostModel>(post), cancellationToken);
    ...
    [HttpPut]
    public async Task<IActionResult> UpdatePost([FromBody] CreateUserPostModel post,
        CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        ...
        var createdPost =
            await _postService.UpdatePost(userId, post.Id, _mapper.Map<UserPostModel>(post), cancellationToken);

        _logger.LogInformation("Post updated");

        return Ok(_mapper.Map<BasePostViewModel>(createdPost));
    }
    ...
    [HttpGet]
    public async Task<IActionResult> GetPosts([FromQuery] PaginationModel paginationModel,
        CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        ...
        return Ok(_mapper.Map<IEnumerable<BasePostViewModel>>(posts.Data));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Ok(_mapper.Map<BasePostViewModel>(post));
    }
}