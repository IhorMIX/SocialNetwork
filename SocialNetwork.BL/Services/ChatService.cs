...
public class ChatService : IChatService
{
    ...
    private async Task<ChatMember?> GetUserInChatAsync(int userId, int chatId, ChatAccess access, CancellationToken cancellationToken)
    {
    ...
    public async Task CreateP2PChat(int userId, int user2Id, ChatModel chatModel, CancellationToken cancellationToken = default)
    {
        ...
        //p2pAdmin role
        var roleList = new List<Role>(){(await _roleRepository.GetByIdAsync(_roleOptions.RoleP2PAdminId, cancellationToken))!};
    ...
    public async Task<ChatModel> CreateGroupChat(int userId, ChatModel chatModel, CancellationToken cancellationToken = default)
    {
        ...
        var roleList = new List<Role>
        {
            ...
            (await _roleRepository.GetByIdAsync(_roleOptions.RoleEveryone, cancellationToken))!
        };
    ...
    public async Task AddUsers(int userId, int chatId, List<int> userIds, CancellationToken cancellationToken = default)
    {
        ...
        {
            var tempUserDb = await _userRepository.GetByIdAsync(id, cancellationToken);
            if (tempUserDb is null)
                idsToRemove.Add(id);
        }
        ...
        var roleList = new List<Role>(){(await _roleRepository.GetByIdAsync(_roleOptions.RoleEveryone, cancellationToken))!};
        var usersToAdd = await _userRepository.GetAll().Where(i => idsToAdd.Contains(i.Id)).ToListAsync(cancellationToken);
    ...
    public async Task DelMember(int userId, int chatId, List<int> userIds, CancellationToken cancellationToken = default)
    {
        var userDb = await _userRepository.GetByIdAsync(userId, cancellationToken);
        _logger.LogAndThrowErrorIfNull(userDb, new UserNotFoundException($"User with this Id {userId} not found"));
        var chatDb = await _chatRepository.GetByIdAsync(chatId, cancellationToken);
        ...
        _logger.LogAndThrowErrorIfNull(membersToDel, new ChatNotFoundException($"Chat with this Id {chatId} not found"));
    ...
    public async Task AddRole(int userId, int chatId, RoleModel roleModel, CancellationToken cancellationToken = default)
    {
        ...
        var isAlreadyNamed = await _roleRepository.GetAll().Where(r => r.Chat!.Id == chatId && r.RoleName == roleModel.RoleName).FirstOrDefaultAsync(cancellationToken);
    ...
    public async Task SetRole(int userId, int chatId, int roleId, List<int> userIds, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var uId in userIds)
        {
            ...
            _logger.LogAndThrowErrorIfNull(chatMemberDb, new UserNotFoundException($"User with this Id {uId} not found"));
